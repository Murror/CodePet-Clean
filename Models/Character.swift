...
struct CharacterRecommender {
    static func recommend(who: String, desire: String, goal: String) -> String {
        ...
        // Who are you?
        switch who {
        case "beginner": scores["luna"]! += 3; scores["byte"]! += 1; scores["sage"]! += 1
        case "idea":     scores["crash"]! += 3; scores["luna"]! += 1; scores["null"]! += 1
        case "builder":  scores["crash"]! += 2; scores["byte"]! += 2; scores["zero"]! += 2
        case "creative": scores["nova"]! += 2; scores["byte"]! += 2; scores["glitch"]! += 2
        default: break
        }

        // What drives you?
        switch desire {
        case "prove":      scores["luna"]! += 2; scores["crash"]! += 1; scores["sage"]! += 1
        case "autonomous": scores["crash"]! += 2; scores["byte"]! += 2; scores["zero"]! += 1
        case "mastery":    scores["nova"]! += 3; scores["sage"]! += 2
        case "speed":      scores["crash"]! += 3; scores["zero"]! += 2
        default: break
        }

        // What's your goal?
        switch goal {
        case "launch":    scores["crash"]! += 2; scores["luna"]! += 1; scores["glitch"]! += 1
        case "portfolio": scores["luna"]! += 2; scores["nova"]! += 1; scores["sage"]! += 1
        case "automate":  scores["byte"]! += 2; scores["zero"]! += 3; scores["crash"]! += 1
        case "levelup":   scores["nova"]! += 3; scores["sage"]! += 2; scores["byte"]! += 1
        default: break
        }
