...
struct CosmeticShopView: View {
                    ...
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                        ForEach(filteredItems) { item in
                            CosmeticItemCard(
                                ...
                                isEquipped: gameState.equippedCosmetics[selectedCategory.rawValue] == item.id,
                                action: {
                                    selectedItem = item
    ...
    private func buyItem(_ item: CosmeticItem) {
        if let coinCost = item.coinCost, gameState.coins >= coinCost {
            gameState.coins -= coinCost
            gameState.ownedCosmetics.insert(item.id)
            gameState.equippedCosmetics[selectedCategory.rawValue] = item.id
        }
    }
